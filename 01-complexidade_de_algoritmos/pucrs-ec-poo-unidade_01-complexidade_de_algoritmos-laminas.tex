...
Notação O
◼
 n2 , (3/2)n2 , n2+100n , crescem com a mesma velocidade
Notação O
◼
 Assintótico
◼
 Quando se quer descrever o comportamento quando
se aproxima de um limite, ou seja, quando n tende ao
infinito
◼
 Sejam f(n) e g(n) funções mapeando inteiros não
negativos em números reais
◼
 Diz-se que f(n) é O(g(n)) se existe uma constante
c > 0 e uma constante n0 >= 1 tais que
f(n) <= cg(n), para todo inteiro n>= n0
◼
 Notação O:
◼
 f(n) é O de g(n)
...
◼
 f(n)=O(g(n)) significa
◼
 que g(n) cresce mais (ou da mesma forma que) a
função f(n) à medida que n cresce para o infinito. Para
isso, pode ser preciso multiplicar g(n) por uma
constante.
◼
 Notação O é usada para:
◼
 Caracterizar o tempo de execução e limites espaciais
em função de um parâmetro n que varia de problema
para problema
© 2010 Goodrich, Tamassia
Notação O
◼
Exemplo: algoritmo para encontrar o maior
elemento de um arranjo de inteiros
◼
 n representa o número de elementos no arranjo
◼
 Usando a notação O se pode afirmar (independente do
computador que será usado):
◼
 Este algoritmo executa em tempo O(n)
◼
 Justificativa:
◼
 O número de operações primitivas executadas pelo algoritmo é
constante em cada iteração
◼
 Pode-se dizer que o tempo de execução do algoritmo para uma
entrada de tamanho n é no máximo uma constante, vezes n
© 2010 Goodrich, Tamassia
Notação O
◼
 Propriedades da notação O
◼
 Permite ignorar os fatores constantes e os termos de
menor ordem, focando nos principais componentes da
função que afetam seu crescimento
◼
 Se f(n) é um polinômio de grau d, isto é,
f(n) = a0 + a1n + ... + adnd
e ad>0, então f(n) é O(nd)
◼
...
Propriedades da notação O
◼
 Portanto, o termo de mais alto grau em um polinômio é
o termo que determina a taxa de crescimento
assintótico do polinômio
◼
 Exemplos:
◼
 5n2 + 3n log n + 2n + 5 é O(n2)
◼
 20n3 + 10n log n + 5 é O(n 3)
◼
 3 log n + 2 é O(log n)
◼
 2n+2 é O(2 n)
...
Notação O
◼
 As sete funções apresentadas são as mais usadas
em conjunto com a notação O
◼
 1
 log n
 n
 ...
 a n
◼
 Caracterizam os tempos de execução e consumo de
memória dos algoritmos
◼
 Os nomes destas funções são usados para referenciar
o tempo de execução
◼
 Exemplos:
◼
 Algoritmo que executa no limite superior em tempo 4n2 + n log n
é um algoritmo de tempo quadrático, pois executa em tempo
O(n2)
◼
 Algoritmo que executa no limite superior em 5n + 20 log n + 4 é
um algoritmo de tempo linear, ou seja, O(n)
© 2010 Goodrich, Tamassia
Roteiro
◼
 Notação O
...
◼
 Análise assintótica
PUCRS, Escola Politécnica - Isabel H. Manssour
Este material é de uso exclusivo dos alunos matriculados na disciplina de Algoritmos e Estruturas de Dados I.
É proibida sua reprodução e divulgação para qualquer finalidade.
Notação 
◼
 Notação O
◼
 Maneira assintótica de dizer que uma função é “menor
que ou igual a” outra função
◼
 Notação 
◼
 Maneira assintótica de dizer que uma função cresce a
uma taxa que é “maior ou igual a” de outra função
◼
Notação 
◼
 Notação  permite dizer que duas funções crescem à
mesma taxa
© 2010 Goodrich, Tamassia
Notação 
◼
...
negativos em números reais
◼
 Diz-se que f(n) é (g(n)) se g(n) é O(f(n)), ou
seja, se existe uma constante c > 0 e uma
constante n0 >= 1 tais que
f(n) >= cg(n), para todo inteiro n>= n0
◼
 Permite dizer que uma função é assintoticamente maior
que ou igual a outra, exceto por um fator constante
◼
 Se diz que f(n) é ômega de g(n)
◼
 Exemplo: 3n log n + 2n é (n log n)
© 2010 Goodrich, Tamassia
Notação 
◼
 Diz-se que f(n) é (g(n)),
se f(n) é O(g(n)) e f(n) é (g(n)), ou seja,
existem constantes reais c’>0 e c’’>0 e uma
constante inteira n 0 >= 1 tais que
c’g(n) <= f(n) <= c’’g(n), para n>= n 0
◼
 Se diz que f(n) é theta de g(n)
◼
 Exemplo: 3n log n <= 4n + 5 log n é (n log n)
© 2010 Goodrich, Tamassia
Notação 
◼
 Exemplo de gráficos das notações O,  e 
◼
 Notação  limita uma função entre valores constantes
◼
 Notação O dá um limite superior para uma função
dentro de um valor constante
◼
 Notação  dá um limite inferior para uma função dentro
...
Análise assintótica
◼
 Suponha dois algoritmos, A e B, para resolver um
mesmo problema
◼
 A tem um tempo de execução O(n)
◼
 B tem um tempo de execução O(n2)
◼
 Qual deles é melhor?
◼
...
Análise assintótica
◼
 Pode-se usar a notação O para ordenar classes
de funções por seu crescimento assintótico
1
 log n
 n
 n log n
...
Análise assintótica
◼
 Para analisar algoritmos com a notação O, o mais
apropriado é dizer:
f(n) é O(g(n)),
ou
f(n)  O(g(n))
...
◼
 Em geral, um algoritmo executando em tempo
O(n log n), com um fator constante razoável, pode
ser considerado eficiente
◼
 Até um método O(n2) pode ser suficientemente
rápido em alguns contextos (n pequeno)
◼
 Algoritmo executando em tempo O(2n) dificilmente
pode ser considerado eficiente
© 2010 Goodrich, Tamassia
Análise assintótica
◼
 Notações O,  e  fornecem uma linguagem
conveniente para análise de estruturas de dados
e algoritmos
◼
 Permitem a concentração nos aspectos gerais, em vez
dos detalhes
© 2010 Goodrich, Tamassia
Referências
◼
...
5a edição
Capítulo 4 (páginas 162 à 182)
Leitura
fortemente
recomendada
◼
 Livro do Cormen et al.
